    ...
    for change in stream:
        ...
        if change['operationType'] == 'update':
            ...
            if 'isBuyThreshold' in change['updateDescription']['updatedFields']:
                ...
                embed = DiscordEmbed(title='ETH H12 - Liquidation des shorts', description=f'Liquidation anormale des shorts : **{buyVolUsd}$** liquidé lors des 12 dernières heures.', color='E11A00')
            ...
            if 'isSellThreshold' in change['updateDescription']['updatedFields']:
                ...
                embed = DiscordEmbed(title='ETH H12 - Liquidation des longs', description=f'Liquidation anormale des longs : **{sellVolUsd}$** liquidé lors des 12 dernières heures.', color='32BD3F')
                webhook.add_embed(embed)
                webhook.set_content('@here')
                response = webhook.execute(remove_embeds=True)
